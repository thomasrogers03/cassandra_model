...
def setup_keyspace(connection_name)
  ...
  connection = CassandraModel::ConnectionCache[connection_name]
  unless connection.keyspace
    session = connection.cluster.connect
    session.execute(base_query % 3)
    session.close
  end
end

def truncate_tables(connection_name)
  connection = CassandraModel::ConnectionCache[connection_name]
  connection.keyspace.tables.each do |table|
    unless table.name == CassandraModel::TableDescriptor.table.name
      connection.session.execute("TRUNCATE #{table.name}")
    end
  end
end
...
RSpec.configure do |config|
  ...
  CassandraModel::ConnectionCache[:single].config = {
      hosts: %w(cassandra.single.dev),
      compressor: :snappy, keyspace: 'integration'
  }
  ...
  config.after do
    truncate_tables(:replicating)
    truncate_tables(:single)
  end
